...
class MainActivity : ComponentActivity() {
    ...
    private fun buildUi() {
        ...
        baseUrlInput = EditText(this)
        pinInput = EditText(this)
        siteIdInput = EditText(this)
        dateInput = EditText(this)
        remarkInput = EditText(this)
        siteList = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
        }

        baseUrlInput.hint = "Render URL，例如 https://telegram-site-record-proxy.onrender.com"
        pinInput.hint = "WEB_ADMIN_PIN"
        siteIdInput.hint = "先按同步地盤，再選地盤"
        dateInput.hint = "日期，例如 20260608"
        remarkInput.hint = "備注，例如 泥水完成"
        ...
        val yesterdayButton = Button(this).apply {
            text = "昨日"
            setOnClickListener {
        ...
        val syncSitesButton = Button(this).apply {
            text = "同步地盤"
            setOnClickListener {
                saveSettings()
                fetchSites()
        ...
        val dateRow = LinearLayout(this).apply {
            ...
            addView(pickDateButton)
            addView(todayButton)
            addView(yesterdayButton)
        }
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    currentSite?.let {
                        ...
                        statusText.text = "已同步地盤：${it.getString("name")}"
                    }
    ...
    private fun renderSiteButtons(sites: JSONArray) {
        ...
        for (index in 0 until sites.length()) {
            ...
            siteList.addView(Button(this).apply {
                text = name
                setOnClickListener {
    ...
    private fun loadSettings() {
        ...
        baseUrlInput.setText(prefs.getString("baseUrl", "https://telegram-site-record-proxy.onrender.com"))
        pinInput.setText(prefs.getString("pin", ""))
        siteIdInput.setText(prefs.getString("siteId", ""))
        dateInput.setText(prefs.getString("date", todayString(0)))
        remarkInput.setText(prefs.getString("remark", ""))
    }
    ...
    private fun hasRequiredUploadSettings(): Boolean {
        if (baseUrlInput.text.isBlank() || pinInput.text.isBlank() || siteIdInput.text.isBlank()) {
            statusText.text = "請先填 Render URL、PIN，並選擇地盤。"
            return false
        }
